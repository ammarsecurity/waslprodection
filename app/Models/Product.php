   ...
   public function video(): Attribute
{
    ...
    if (
        $this->videoMedia &&
        $this->videoMedia->type == 'file' &&
        file_exists(public_path('storage/' . $this->videoMedia->src))
    ) {
        $video = (object) [
            'id' => $this->videoMedia->id,
            'url' => asset('storage/' . $this->videoMedia->src),
   ...
   public function thumbnails(): Collection
{
    ...
    if (request()->is('api/*')) {
        // حالة الفيديو من نوع ملف
        if (
            $this->videoMedia &&
            $this->videoMedia->type == 'file' &&
            file_exists(public_path('storage/' . $this->videoMedia->src))
        ) {
            $path = 'storage/' . $this->videoMedia->src;
        ...
        // حالة الفيديو من نوع رابط خارجي (يوتيوب، فيمو.. إلخ)
        elseif ($this->videoMedia && $this->videoMedia->type != 'file' && $this->videoMedia->src != null) {
            $thumbnails[] = (object) [
                'id' => $this->videoMedia->id,
                'thumbnail' => null,
        ...
        $thumbnails[] = (object) [
            'id' => $this->media?->id,
            'thumbnail' => $this->thumbnail, // هاي تستدعي دالة thumbnail()
            'url' => null,
            'type' => 'image',
        ];
    }

    // بقية الصور المرتبطة (medias)
    foreach ($this->medias as $media) {
        $thumbnail = asset('default/default.jpg');

        if ($media && file_exists(public_path('storage/' . $media->src))) {
            $thumbnail = asset('storage/' . $media->src);
        }
