...
class OrderRepository extends Repository
{
    ...
    private static function createNewOrder($request, $shop, $paymentMethod, $getCartAmounts, $isAgent = false)
    {
        ...
        // Use custom delivery charge if provided, otherwise use calculated one
        $deliveryCharge = $request->delivery_charge ?? $getCartAmounts['deliveryCharge'];
        ...
        try {
            ...
            $telegramService->sendOrderNotification($order, 'new');
        } catch (\Throwable $th) {
            // Log error but don't break the order creation process
            \Log::error('Telegram notification failed for order ' . $order->id . ': ' . $th->getMessage());
        }
    ...
    public static function reOrder(Order $order, $payment): Order
    {
        ...
        // Send Telegram notification for re-order
        try {
            $telegramService = new TelegramNotificationService();
            $telegramService->sendOrderNotification($newOrder, 'reorder');
        } catch (\Throwable $th) {
            // Log error but don't break the re-order process
            \Log::error('Telegram notification failed for re-order ' . $newOrder->id . ': ' . $th->getMessage());
        }
