   ...
   public function index(Request $request)
{
    ...
    $products = ProductRepository::query()
        ...
        ->when($search, function ($query) use ($search) {
            ...
            return $query->whereHas('categories', function ($query) use ($categoriesArray) {
                $query->whereIn('id', $categoriesArray);
            });
            ...
            return $query->whereHas('subcategories', function ($query) use ($subCategoriesArray) {
                $query->whereIn('id', $subCategoriesArray);
            });
    ...
    /**
     * Get discounted products for a shop
     */
    public function discounted(Request $request)
    {
        ...
        $products = ProductRepository::query()
            ...
            ->limit(12)
            ->get();
        return $this->json('discounted products', [
            'products' => ProductResource::collection($products),
        ]);
    }

    /**
     * Get new products for a shop
     */
