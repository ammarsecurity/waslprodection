...
class ChatController extends Controller
{
        ...
        if ($this->isProductQuery($message)) {
            ...
            if ($products->count()) {
                ...
                foreach ($products as $product) {
                    $reply .= "{$product->name} - {$product->price} {$product->currency}\n";
                }
            } else {
                $reply = "لم يتم العثور على منتجات بهذا الوصف.";
            }
        } else {
            $reply = $this->askChatGPT($message);
        }
    ...
    {
        return preg_match('/بدلة|سعر|شحن|منتج|مقاس|زفاف/i', $message);
    }

    private function askChatGPT($message)
    {
        // مثال: استخدم OpenAI API
        ...
        if ($response->successful()) {
            return $response['choices'][0]['message']['content'] ?? 'لم يتم الحصول على رد.';
        }
