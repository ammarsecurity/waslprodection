    ...
    public function cancel(Request $request)
    {
        ...
        if ($order->order_status->value == OrderStatus::PENDING->value) {
            ...
            try {
                ...
                $telegramService->sendOrderStatusChangeNotification($order, $oldStatus, OrderStatus::CANCELLED->value);
            } catch (\Throwable $th) {
                // Log error but don't break the cancellation process
                \Log::error('Telegram cancellation notification failed for order ' . $order->id . ': ' . $th->getMessage());
            }
