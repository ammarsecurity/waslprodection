...
class OrderController extends Controller
{
    ...
    public function statusChange(Order $order, Request $request)
    {
        ...
        try {
            ...
            $telegramService->sendOrderStatusChangeNotification($order, $oldStatus, $request->status);
        } catch (\Throwable $th) {
            // Log error but don't break the status change process
            \Log::error('Telegram status change notification failed for order ' . $order->id . ': ' . $th->getMessage());
        }
    ...
    public function paymentStatusToggle(Order $order)
    {
        ...
        $oldStatus = $order->payment_status->value;
        $order->update(['payment_status' => PaymentStatus::PAID->value]);

        // Send Telegram notification for payment status change
        try {
            $telegramService = new TelegramNotificationService();
            $telegramService->sendPaymentStatusChangeNotification($order, $oldStatus, PaymentStatus::PAID->value);
        } catch (\Throwable $th) {
            // Log error but don't break the payment status change process
            \Log::error('Telegram payment status change notification failed for order ' . $order->id . ': ' . $th->getMessage());
        }
    ...
    $qrCode = new QrCode($orderCode);
        ...
        $view = view('PDF.invoice', compact('order', 'qrCodeImage', 'generaleSetting'))->render();
        $mPdf->WriteHTML($view);
