    ...
    <div>
        <div class="p-6 bg-white rounded-2xl border border-slate-200">
            ...
            <div class="my-4 flex justify-between gap-4">
                ...
                <div class="text-slate-950 text-base font-normal leading-normal">
                    {{ props.showCurrency ? props.showCurrency(props.deliveryCharge || basketStore.delivery_charge) : master.showCurrency(props.deliveryCharge || basketStore.delivery_charge) }}
                </div>
            </div>

            <div v-if="basketStore.all_vat_taxes?.length > 0 || basketStore.order_tax_amount > 0" class="p-3 bg-slate-100 text-black rounded-lg mb-2">
                <h2 class="text-sm sm:text-base font-medium mb-2">
                    {{ $t('VAT & Taxes Summary') }}
                </h2>

                <div class="space-y-2">
                    <div v-for="vatTax in basketStore.all_vat_taxes" :key="vatTax.id" class="flex justify-between bg-slate-200 p-2 rounded-lg">
...
// Calculate total payable including delivery charge
const calculateTotalPayable = () => {
    const subtotal = basketStore.total_amount - basketStore.coupon_discount;
    const shipping = props.deliveryCharge || basketStore.delivery_charge;
    const taxes = basketStore.order_tax_amount || 0;
    const total = subtotal + shipping + taxes;
    return props.showCurrency ? props.showCurrency(total) : master.showCurrency(total);
};
...
const processOrderConfirm = () => {
    ...
    if (basketStore.checkoutProducts.length > 0) {
        ...
        axios.post('/place-order', {
            ...
            note: props.note,
            delivery_charge: props.deliveryCharge,
            ...provinceData
        }, {
            headers: {
