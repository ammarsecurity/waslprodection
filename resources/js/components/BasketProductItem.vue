<template>
    <div class="bg-white flex gap-4 w-full group">
        ...
        <div class="grow flex flex-col gap-2 border-b border-slate-200 pb-3 relative overflow-hidden">
            <!-- Brand (optional) -->
            <div v-if="props.product?.brand" class="text-primary text-xs font-normal leading-none">
            ...
            <!-- Price / Discount (supports guest unit_price) -->
            <div class="flex items-center gap-2">
                <div class="text-primary text-base font-bold leading-normal">
                    {{ master.showCurrency(props.product?.unit_price ?? (props.product?.discount_price > 0 ? props.product?.discount_price : props.product?.price)) }}
                </div>
                <div v-if="!props.product?.unit_price && props.product?.discount_price > 0" class="text-slate-400 text-sm font-normal line-through leading-tight">{{ master.showCurrency(props.product?.price ) }}</div>
                <div v-if="!props.product?.unit_price && props.product?.discount_percentage > 0" class="px-1 py-0.5 bg-red-500 rounded-2xl text-white text-xs font-medium">
            ...
            <div class="flex justify-between gap-2 flex-wrap w-full items-center">
                <div class="flex gap-2">
                    <div v-if="props.product?.size || props.product?.size_name" class="min-w-8 p-1 bg-slate-100 rounded text-center text-slate-800 text-xs font-normal">
                       {{ props.product?.size?.name ?? props.product?.size_name }}
                    </div>
                    <div v-if="props.product?.color || props.product?.color_name" class="min-w-8 p-1 bg-slate-100 rounded text-center text-slate-800 text-xs font-normal">
                       {{ props.product?.color?.name ?? props.product?.color_name }}
                    </div>
                </div>
