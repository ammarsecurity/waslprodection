        ...
        <div class="p-3 md:p-4 xl:p-6">
            ...
            <form @submit.prevent="submitForm()">
                <div class="max-w-[920px] mx-auto bg-white rounded-xl md:rounded-2xl p-3 md:p-4 xl:p-6">
                    ...
                    <div v-if="authStore.user?.agent_shops && authStore.user.agent_shops.length > 0" 
                         class="max-w-[920px] mx-auto bg-gradient-to-br from-blue-50 to-indigo-50 rounded-xl md:rounded-2xl p-4 md:p-6 mt-4 border border-blue-200">
                        <div class="flex items-center gap-3 mb-4">
                            ...
                            <div>
                                <h3 class="text-lg font-semibold text-blue-900">{{ $t('Wholesale Agent Account') }}</h3>
                                <p class="text-sm text-blue-700">{{ $t('You are registered as a wholesale agent') }}</p>
                            </div>
                        </div>
                        
                        <div class="bg-white rounded-lg p-4 border border-blue-100">
                            <p class="text-sm text-slate-600 mb-3">
                                {{ $t('You have special wholesale pricing access for the following shops') }}:
                            </p>
                            <div class="flex flex-wrap gap-2">
                                <div v-for="shop in authStore.user.agent_shops" :key="shop.id" 
                                     class="inline-flex items-center gap-2 px-3 py-2 bg-blue-100 text-blue-800 rounded-full text-sm font-medium">
                                    <img v-if="shop.logo" :src="shop.logo" :alt="shop.name" class="w-6 h-6 rounded-full object-cover" />
                            ...
                            <div class="mt-4 pt-4 border-t border-blue-100">
                                <div class="flex items-start gap-2">
                                    ...
                                    <p class="text-sm text-slate-600">
                                        {{ $t('As an agent, you will see special wholesale prices on products from these shops. Contact the shop owner if you have any questions about your agent status.') }}
                                    </p>
                                </div>
                            </div>
                        </div>
...
onMounted(async () => {
    // Refresh user data to get latest agent_shops information
