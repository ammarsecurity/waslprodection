            ...
            <div class="card mt-3">
                ...
                @if($order->province_name || $order->province_area || $order->nearest_landmark)
                    <div class="border-bottom d-flex align-items-center justify-content-between gap-2 px-3 py-12">
                        <span class="text-color">{{ __('Province Name') }}: </span>
                        <span class="fw-medium">{{ $order->province_name ?? '-' }}</span>
                    </div>
                    <div class="border-bottom d-flex align-items-center justify-content-between gap-2 px-3 py-12">
                        <span class="text-color">{{ __('Province Area') }}: </span>
                        <span class="fw-medium">{{ $order->province_area ?? '-' }}</span>
                    </div>
                    <div class="d-flex align-items-center justify-content-between gap-2 px-3 py-12">
                        <span class="text-color">{{ __('Nearest Landmark') }}: </span>
                        <span class="fw-medium">{{ $order->nearest_landmark ?? '-' }}</span>
                    </div>
                @else
                    <!-- Fallback to old address if province fields are empty -->
                    <div class="border-bottom d-flex align-items-center justify-content-between gap-2 px-3 py-12">
                        <span class="text-color">{{ __('Name') }}: </span>
