...
@section('content')
    ...
    <form action="{{ route('admin.subscription-plan.update', $subscriptionPlan->id) }}" method="POST">
        ...
        <div class="card">
            <div class="card-body">
                <div>
                    <x-input label="{{__('Name')}}" name="name" type="text" placeholder="{{__('Enter Name')}}" :value="$subscriptionPlan->name"
                        required="true" />
                </div>
                ...
                <div class="mt-3">
                    ...
                    <textarea name="short_description" class="form-control @error('short_description') is-invalid @enderror"
                        rows="2" placeholder="{{__('Enter short description')}}">{{ old('short_description', $subscriptionPlan->short_description) }}</textarea>
                ...
                <div class="mt-3">
                    <x-input type="text" name="price" label="{{__('Price')}}" placeholder="{{__('Price')}}" :value="$subscriptionPlan->price"
                        required="true" onlyNumber="true" />
                </div>

                <div class="mt-3">
                    <x-input type="text" name="duration" label="{{__('Duration (days)')}}" placeholder="{{__('Duration')}}" :value="$subscriptionPlan->duration"
                        onlyNumber="true" />
                </div>

                <div class="mt-3">
                    <x-input type="text" name="sale_limit" label="{{__('Sell Limit')}}" placeholder="{{__('Sell Limit')}}" :value="$subscriptionPlan->sale_limit"
                        onlyNumber="true" />
                </div>
